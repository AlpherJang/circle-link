...
func NewRouter() http.Handler {
	return NewRouterWithServices(
		...
		contact.NewMemoryService(),
		device.NewMemoryService(),
		message.NewMemoryService(),
	)
}

func NewRouterWithServices(authService auth.Service, contactService contact.Service, deviceService device.Service, messageService message.Service) http.Handler {
	server := &Server{
	...
	mux.HandleFunc("GET /v1/conversations", server.handleListConversations)
	...
	mux.HandleFunc("GET /v1/contacts", server.handleListContacts)
	mux.HandleFunc("POST /v1/contacts/invite", server.handleInviteContact)
	mux.HandleFunc("POST /v1/contacts/{peerUserId}/accept", server.handleAcceptContact)
	mux.HandleFunc("POST /v1/contacts/{peerUserId}/reject", server.handleRejectContact)
